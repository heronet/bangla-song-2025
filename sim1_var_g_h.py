...
def air_density(altitude):
    """
    Calculate air density at given altitude with variable scale height
    
    For variable H, we need to integrate: dρ/dh = -ρ/(H(h))
    This doesn't have a simple closed form, so we use an approximation:
    
    We'll use the average scale height between sea level and altitude h
    ρ(h) ≈ ρ₀ × exp(-h / H_avg)
...
def derivatives(state, t, base_altitude):
    """
    Calculate derivatives for projectile motion with air resistance
    state = [x, y, vx, vy]
    
    Now g and air density vary with altitude
    ...
    # Get gravitational acceleration at current altitude
...
def simulate_trajectory_euler(v0, angle_deg, altitude, dt=0.01, max_time=60):
    """
    Simulate trajectory using Euler method with variable g and air density
    ...
    while y >= 0 and t < max_time and x < MAX_RANGE:
        ...
        # Get variable parameters at current altitude
        g = gravity(current_altitude)
        rho = air_density(current_altitude)
...
def find_solution(target_distance, altitude, max_velocity=450, method='euler'):
    ...
    print(f"Air density at {altitude}m: {air_density(altitude):.4f} kg/m³")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("ARTILLERY TRAJECTORY SIMULATOR WITH VARIABLE g AND ρ")
    print("="*60)
    ...
    if solution:
        print_solution(solution, TARGET_DISTANCE, ALTITUDE)
        ...
        rho_variable = air_density(ALTITUDE)
        ...
        for alt in [0, 500, 1000, 1500, 2000]:
            rho_const = RHO_SEA * np.exp(-alt / SCALE_HEIGHT_SEA)
            rho_var = air_density(alt)
            g_val = gravity(alt)
            H_val = scale_height(alt)
            print(f"  {alt:4d}m: ρ_const={rho_const:.5f}, ρ_var={rho_var:.5f}, g={g_val:.5f}, H={H_val:.1f}m")
        
        print(f"\nUsing accurate numerical integration for {ALTITUDE}m:")
        rho_accurate = air_density_accurate(ALTITUDE, num_steps=1000)
        print(f"  ρ_accurate = {rho_accurate:.6f} kg/m³")
        print(f"  Difference from variable method: {abs(rho_variable - rho_accurate):.6f} kg/m³")
        print("="*60)
        
        plot_trajectory(solution, TARGET_DISTANCE, ALTITUDE)
        
        # Compare with no-drag case
        angle_no_drag = 45  # optimal angle without drag
        v0_no_drag = np.sqrt(TARGET_DISTANCE * gravity(ALTITUDE) / np.sin(2 * np.radians(angle_no_drag)))
        print(f"\nWithout air resistance (at {ALTITUDE}m altitude):")
        print(f"  Required velocity at 45°: {v0_no_drag:.1f} m/s")
        print(f"  With drag requires:       {solution['velocity']:.1f} m/s")
        print(f"  Additional velocity needed: +{solution['velocity'] - v0_no_drag:.1f} m/s ({100*(solution['velocity'] - v0_no_drag)/v0_no_drag:.1f}% increase due to drag)")